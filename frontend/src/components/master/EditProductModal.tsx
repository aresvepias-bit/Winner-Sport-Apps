...
export default function EditProductModal({ product, onClose, onSubmit }: EditProductModalProps) {
  const [form, setForm] = useState({
    ...
    priceSatuan: Number(product.priceSatuan) || 0,
    priceGrosir: Number(product.priceGrosir) || 0,
    priceKodi: Number(product.priceKodi) || 0,
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#0d1424] border border-slate-200 dark:border-[#1a2236] rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-[#1a2236] pb-3">
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">Edit Produk Pakaian Jadi</h3>
            <p className="text-[11px] font-mono text-red-600 dark:text-red-400 font-bold">{product.sku}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          <div>
            <label className="block text-slate-600 dark:text-slate-400 mb-1 font-medium">Nama Baju / Model Produk</label>
            <input
              type="text"
              value={form.name}
          ...
          <div>
            <label className="block text-slate-600 dark:text-slate-400 mb-1 font-medium">Deskripsi / Spesifikasi Bahan</label>
            <textarea
              rows={2}
              value={form.description}
              onChange={(e) => setForm({ ...form, description: e.target.value })}
              placeholder="Pola regular fit, jenis sablon, kerah rib..."

...
export type MasterTabType = "materials" | "products" | "boms" | "employees" | "units" | "salesTypes";

const GROUPS = ["Katalog & Produksi", "Tim & Karyawan", "Pengaturan Dasar"];
export const MASTER_MENUS = [
  { key: "materials", label: "Bahan Baku", description: "Kain, aksesoris, dan kebutuhan produksi.", keywords: "stok inventory", group: GROUPS[0], icon: Package },
  { key: "products", label: "Produk Jadi", description: "Katalog pakaian dan stok siap jual.", keywords: "pakaian model", group: GROUPS[0], icon: Layers },
  { key: "boms", label: "Formula / BOM", description: "Komposisi bahan dan resep produksi pakaian.", keywords: "resep jahit spesifikasi", group: GROUPS[0], icon: Scissors },
  { key: "employees", label: "Penjahit & Karyawan", description: "Data tim produksi dan pengaturan upah.", keywords: "pegawai staff borongan", group: GROUPS[1], icon: UserRound },
  { key: "units", label: "Satuan", description: "Satuan untuk pencatatan barang dan transaksi.", keywords: "unit pcs meter kg", group: GROUPS[2], icon: Ruler },
  { key: "salesTypes", label: "Tipe Penjualan", description: "Kategori penjualan untuk kebutuhan usaha.", keywords: "retail grosir", group: GROUPS[2], icon: Tags },
...
export default function MasterNavTabs({ activeTab, onTabChange }: MasterNavTabsProps) {
  ...
  return (
    <nav aria-label="Menu master database" className="self-start overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-[#1a2236] dark:bg-[#0d1424] xl:sticky xl:top-20">
      <div className="hidden items-center gap-3 border-b border-slate-100 px-5 py-4 dark:border-[#1a2236] xl:flex">
        ...
        <h2 className="text-sm font-bold">Setup Master</h2>
        <span className="ml-auto rounded-md bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-500 dark:bg-slate-800 dark:text-slate-400">{MASTER_MENUS.length}</span>
      </div>
      ...
      <div id="master-menu-list" className={`${expanded ? "block" : "hidden"} xl:block`}>
        <div className="px-3 pt-3">
          <div className="relative">
            ...
            <input aria-label="Cari menu master" placeholder="Cari menu master..." value={search} onChange={(event) => setSearch(event.target.value)} className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-9 text-xs outline-none focus:border-red-400 focus:ring-2 focus:ring-red-500/10 dark:border-slate-700 dark:bg-[#090e1a]" />
        ...
        <div className="max-h-[min(60vh,36rem)] space-y-5 overflow-y-auto p-3 xl:max-h-[calc(100dvh-16rem)]">
          ...
          {!filtered.length && <div role="status" className="px-2 py-6 text-center"><p className="text-sm font-semibold">Menu tidak ditemukan</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Coba kata lain, seperti bahan atau satuan.</p><button type="button" onClick={() => setSearch("")} className="mt-3 text-xs font-semibold text-red-600 dark:text-red-400">Tampilkan semua menu</button></div>}
        </div>
        <p className="hidden border-t border-slate-100 px-5 py-4 text-xs leading-relaxed text-slate-500 dark:border-[#1a2236] dark:text-slate-400 xl:block">Pilih kategori atau cari menu untuk mengelola data master.</p>
      </div>
    </nav>
  );
}
